...
class Tadano_Summarizer(object):
    ...
    def __join_clusters(self, aspect):
        ...
        for i in range(clusters_size):
            for j in range(i+1, clusters_size):
                common_words =  self.__has_common_words(list(self.__clusters[aspect][clusters[i]]['representative_words']),
                                                        list(self.__clusters[aspect][clusters[j]]['representative_words']))
                if common_words:
    ...
    def create_summary(self, folder_path, number_aspects, number_sentences, write=True):
        ...
        file_out = open(os.path.join(folder_path, self.__name + "_Tadano.sum"), 'w') 
        ...
        for aspect, frequency in top_aspects[:number_aspects]:
            ...
            print("NUM_Aspectos:", number_aspects)
            print("NUM_Sentencas:", number_sentences)
            #print("Aspecto:", aspect)
            #print(sentences)
                
            file_out.write("Aspecto: " + aspect + '\n')
            for i in range(len(sentences)):
                file_out.write(sentences[i] + '\n')
            file_out.write('\n')
